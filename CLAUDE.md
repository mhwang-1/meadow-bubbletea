...
Personal task management TUI app using a timebox-first approach. Built with Go and BubbleTea, served via ttyd in the browser, with Telegram bot integration. All data stored as markdown files.
...
./meadow serve                  # Start Telegram bot (requires env vars)
...
internal/
  ...
  telegram/                      # Telegram bot
    bot.go                       # Bot setup, long polling, chat ID auth
    handlers.go                  # /today, /week, /lists, /done, /add, etc.
...
Unit tests cover the domain layer (duration parsing, week calculation, task sequencing) and store layer (read/write round-trips for all file types). Run with `go test ./...`.

## Docker

```bash
docker compose up --build       # Build and start (port 34135)
```

The container runs ttyd on port 34135 and optionally starts the Telegram bot if `TELEGRAM_BOT_TOKEN` is set.
