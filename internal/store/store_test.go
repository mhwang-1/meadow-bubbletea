...
func TestCompletedDurationsBySlug(t *testing.T) {
	tmpDir := t.TempDir()
	s := NewStore(tmpDir)

	// Add completed tasks for week 13.
	tasks := []CompletedTask{
		{Task: domain.Task{Description: "Task A", Duration: 30 * time.Minute}, CompletedDate: time.Date(2026, 3, 23, 0, 0, 0, 0, time.UTC), TaskListSlug: "work"},
		{Task: domain.Task{Description: "Task B", Duration: 1 * time.Hour}, CompletedDate: time.Date(2026, 3, 24, 0, 0, 0, 0, time.UTC), TaskListSlug: "work"},
		{Task: domain.Task{Description: "Task C", Duration: 45 * time.Minute}, CompletedDate: time.Date(2026, 3, 25, 0, 0, 0, 0, time.UTC), TaskListSlug: "personal"},
	}
	if err := s.WriteCompleted(2026, 13, tasks); err != nil {
		t.Fatalf("WriteCompleted: %v", err)
	}

	got, err := s.CompletedDurationsBySlug(2026, 13)
	if err != nil {
		t.Fatalf("CompletedDurationsBySlug: %v", err)
	}

	if got["work"] != 90*time.Minute {
		t.Errorf("work: got %v, want 1h30m", got["work"])
	}
	if got["personal"] != 45*time.Minute {
		t.Errorf("personal: got %v, want 45m", got["personal"])
	}

	// Non-existent week returns empty map.
	empty, err := s.CompletedDurationsBySlug(2026, 1)
	if err != nil {
		t.Fatalf("CompletedDurationsBySlug (empty): %v", err)
	}
	if len(empty) != 0 {
