...
type TaskListEditorModel struct {
	...
	executeViewport scrollViewport    // scrollable viewport for Execute mode
...
// executeViewportHeight returns the usable viewport height for Execute mode.
func (m RootModel) executeViewportHeight() int {
	contentHeight := m.height - 3 // header + separator + bottom margin
	if contentHeight < 3 {
		contentHeight = 3
	}

	vpHeight := contentHeight - 4 // hints (1) + blank (1) + stats below (2)
	if vpHeight < 3 {
		vpHeight = 3
	}
	return vpHeight
...
// syncExecuteViewportSize updates Execute mode viewport dimensions for the current terminal size.
func (m *RootModel) syncExecuteViewportSize() {
	m.taskListEditor.executeViewport.Width = m.width
	m.taskListEditor.executeViewport.Height = m.executeViewportHeight()
	m.taskListEditor.executeViewport.SetYOffset(m.taskListEditor.executeViewport.YOffset)
}
...
func (m *RootModel) openTaskListEditor(slug string) {
	...
	// Initialise execute mode viewport.
	execVpHeight := m.executeViewportHeight()
	m.taskListEditor.executeViewport = newScrollViewport(m.width, execVpHeight)
...
func (m *RootModel) closeTaskListEditor() {
	...
	// Return to the originating tab in the task list menu.
	switch m.editorOriginTab {
	case MenuTabHistory:
		m.taskListMenuTab = MenuTabHistory
		m.historySelectedYear = m.editorOriginYear
		m.historySelectedWeek = m.editorOriginWeek
		m.loadHistoryData()
		// Restore year/week selection after loading.
		m.historySelectedYear = m.editorOriginYear
		m.historySelectedWeek = m.editorOriginWeek
	...
	case MenuTabArchived:
		...
		m.showTaskListMenu = true
	default:
		// Active tab: re-open the menu.
		m.openTaskListMenu()
	}
}

// openArchivedTaskListEditor opens an archived task list in read-only Archive mode.
func (m *RootModel) openArchivedTaskListEditor(year, week int, slug string) error {
	archivePath := filepath.Join(m.store.ArchiveDir(year, week), "tasklists", slug+".md")
	completedPath := filepath.Join(m.store.ArchiveDir(year, week), "completed.md")
	...
	if err == nil {
		...
		completed, readCompletedErr := m.store.ReadCompleted(year, week)
		if readCompletedErr != nil {
			return fmt.Errorf("read completed tasks at %s: %w (snapshot lookup at %s failed: %v)", completedPath, readCompletedErr, archivePath, err)
		}
		...
		if !found {
			return fmt.Errorf("no completed entries for slug %q in %s (snapshot missing at %s)", slug, completedPath, archivePath)
		}

		// History entries come from completed.md and can exist even when the
		// archived task list snapshot file for that exact week is missing.
...
// ensureExecuteCursorVisible adjusts the execute viewport so the cursor is visible.
func (m *RootModel) ensureExecuteCursorVisible() {
	linePos := m.taskListEditor.executeCursor
	vpHeight := m.executeViewportHeight()
	offset := m.taskListEditor.executeViewport.YOffset

	if linePos+1 > offset+vpHeight {
		m.taskListEditor.executeViewport.SetYOffset(linePos + 1 - vpHeight)
	}
	if linePos < offset {
		m.taskListEditor.executeViewport.SetYOffset(linePos)
	}
}
...
func (m *RootModel) handleExecuteModeKey(key string) tea.Cmd {
	...
	switch key {
	case "pgup":
		m.taskListEditor.executeViewport.PageUp()
		return nil

	case "pgdown":
		m.taskListEditor.executeViewport.PageDown()
		return nil
...
func (m *RootModel) renderTaskListEditor() string {
	...
	case EditorModeExecute:
		b.WriteString(m.renderExecuteMode())
...
// renderExecuteMode renders the Execute mode view through a viewport.
func (m *RootModel) renderExecuteMode() string {
	var b strings.Builder
	...
	// Build task list body for viewport.
	...
	for i, task := range activeTasks {
		...
		if i == m.taskListEditor.executeCursor {
			body.WriteString(highlightStyle.Render(line))
		} else {
			body.WriteString(normalStyle.Render(line))
		}
		body.WriteString("\n")
	}
	...
	m.syncExecuteViewportSize()
	m.taskListEditor.executeViewport.SetContent(body.String())
	scrollLabel := m.taskListEditor.executeViewport.Indicator()

	// Shortcut hints (fixed above viewport).
	hint := scrollLabel + " \u00b7 \u2191\u2193 navigate \u00b7 Enter/x mark done \u00b7 Esc close"
	b.WriteString(hintStyle.Render(truncate(hint, m.width)))
	b.WriteString("\n\n")

	b.WriteString(m.taskListEditor.executeViewport.View())

	// Stats line at bottom (fixed below viewport).
	b.WriteString("\n")
	statsLine := m.calculateTaskListStats(tl)
...
func (m *RootModel) renderArchiveMode(height int) string {
	...
	hintStyle := ui.ShortcutBarStyle
	if m.taskListEditor.readOnly {
		b.WriteString(hintStyle.Render("[/] year \u00b7 \u2190\u2192 week \u00b7 \u2191\u2193 select \u00b7 Esc close"))
	} else {
		b.WriteString(hintStyle.Render("[/] year \u00b7 \u2190\u2192 week \u00b7 \u2191\u2193 select \u00b7 u unmark \u00b7 Esc close"))
	}
	b.WriteString("\n\n")
